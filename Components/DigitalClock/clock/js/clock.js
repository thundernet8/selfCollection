...
function getDigitalTime() {
	...
	return date.getFullYear() + '-' + (date.getMonth()+1) + '-' +date.getDay() + ' ' + date.getHours() + ' : ' + (date.getMinutes()+100).toString().substr(1,2) + ' : ' + (date.getSeconds()+100).toString().substr(1,2);
}
...
function scaleCoordinates(delta, firstTime) {
	$('#ticker, #timelable, #timeleft, #digitaltime, .clock-number').each(function() {
		//Get X,Y,font size
...
$(document).ready(function() {
	...
	$("#canvas").everyTime("1s", function(i) {
		...
		$('#digitaltime').text(getDigitalTime());
	});
